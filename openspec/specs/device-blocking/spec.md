...
The app SHALL persist the list of blocked devices in a local Room database. The blocked state SHALL survive app restarts and device reboots. Each record SHALL store at minimum the device MAC address, device name at the time of blocking, a nullable CDM association ID (`cdmAssociationId: Int?`), a temporary-allow flag (`isTemporarilyAllowed: Boolean`, default false), and an alert-enabled flag (`isAlertEnabled: Boolean`, default false). The database schema is at version 1.
